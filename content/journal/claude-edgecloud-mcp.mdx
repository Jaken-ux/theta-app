...
The next experiment: a multi-step pipeline. Generate an image on Theta. Transcribe an audio file on Theta. Have Claude tie the outputs together into a single artifact. If MCP delivers on the orchestration promise, that should also work in a single chat.

I'll write that up here when I've got it running.
