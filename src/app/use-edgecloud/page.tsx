...
export default async function UseEdgeCloudPage() {
  ...
  return (
    <div className="space-y-12">
      {/* Accuracy warning — sits above everything so a visitor never
          sees the playground without first seeing the disclaimer.
          Stays in place even after dismiss-style banners would be
          missed; not collapsible by design while we work through
          hallucination issues. */}
      <div
        role="alert"
        className="flex items-start gap-3 bg-amber-400/10 border border-amber-400/30 rounded-xl px-4 py-3.5 text-amber-200"
      >
        <svg
          className="w-5 h-5 mt-0.5 flex-shrink-0 text-amber-300"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
