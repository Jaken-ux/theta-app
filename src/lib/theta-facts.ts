...
export const THETA_FACTS = {
  ...
  absorptionMethod: {
    value:
      "TFUEL absorption rate is computed via the SUPPLY-DELTA method, NOT by sampling on-chain burn logs. Method: read total TFUEL circulating supply from the Theta Explorer API at two daily snapshots (a fixed UTC cron, no longer triggered by site traffic), compute supply growth (delta), derive raw absorption = (fixed daily block issuance of 1,238,400 TFUEL) − (supply growth). Daily bars are RAW — there is NO per-bar rolling smoothing as of 2026-06-28 (the previous 3-day centered average was removed when the snapshot-timing drift problem was solved at the source). Safety rails: pre-flagged historical artifact dates (Apr 21, Apr 24, Apr 27 2026 — all pre-cron-fix) are clamped to 0, and any day where the raw value is below 0% or above 100% (physically implausible) is also clamped to 0 and flagged as an artifact. The headline figure is a 7-DAY TRAILING average over the clean (non-artifact) daily values; that is the ONLY smoothing applied anywhere. DO NOT describe absorption as 'sum of burned tokens divided by minted tokens' or 'reading TFUEL burned from on-chain logs' — both are wrong methodology. DO NOT claim a 3-day rolling average is applied — it was removed. For full details see thetasimplified.com/methodology.",
